...
<div class="mb-6 py-3 px-0 sm:px-4">
	<div class={`relative ${isDoc ? 'border-bottom' : 'border'} iframe-container`}>
		{#if $showSpinner$}
			<div class="absolute top-50 start-50 translate-middle iframeSpinner">
	...
	{#if showButtons}
		<div class="border border-t-0 flex items-center p-1" role="toolbar" aria-label="Toolbar with button groups">
			<button
	...
	{#if showCode}
		{#if files.length > 1}
			<div role="tablist" class="tabs tabs-border p-3 border-x-1 border-gray-300">
				{#each files as file}
					<button class={['tab', {'tab-active': selectedFileName === file}]} onclick={() => (selectedFileName = file)}>{file}</button>
				{/each}
			</div>
		{/if}
		<div>
