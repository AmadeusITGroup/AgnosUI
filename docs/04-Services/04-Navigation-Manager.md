...
- Utilities to navigate the items:
  ...
  - `focusPrevious` - method to focus the previous element, respecting the anscestor direction
  - `focusNext` - method to focus the next element, respecting the anscestor direction
  - `focusFirst` - method to focus the first element, respecting the anscestor direction
  - `focusFirstLeft` - method to focus the element at the left-end of the list
  - `focusFirstRight` - method to focus the element at the right-end of the list
  - `focusLast` - method to focus the last element, respecting the anscestor direction
  - `focusLeft` - method to focus the next focusable element to the left of the currently focused element
  - `focusRight` - method to focus the next focusable element to the right of the currently focused element
...
- use the `NavManagerItemConfig` to configure the NavManger by setting:
  - `keys` - a map of key handlers. The key in the map should match the key event (for example, "ArrowLeft" or "Ctrl+PageDown"). The value in the map is the corresponding key handler. (for example, "FocusNext" or "FocusPrevious")
