...
export function createSlider(config) {
    ...
    const values$ = bindableDerived(onValuesChange$, [_dirtyValues$], ([dirtyValues]) => dirtyValues.map((dv) => computeCleanValue(dv)), typeArray.equal);
    // computed
    ...
    const computeCleanValue = (value) => {
        const min = min$(), max = max$(), stepSize = stepSize$();
        if (value >= max) {
            return max;
        }
        else if (value <= min) {
