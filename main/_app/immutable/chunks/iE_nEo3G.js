...
function getCollapseDefaultConfig() {
  return { ...collapse$1.getCollapseDefaultConfig(), ...defaultConfigExtraProps };
}
const createCollapse = widget.createWidgetFactory("collapse", (config) => {
  ...
  const transitionFn$ = tansu.asWritable(tansu.computed(() => horizontal$() ? collapse.collapseHorizontalTransition : collapse.collapseVerticalTransition));
  const widget2 = collapse$1.createCollapse({ ...config ?? {}, props: { ...config?.props ?? {}, transition: transitionFn$ } });
  return {
    ...widget2,
