...
function createCollapse(config) {
  const [{ animatedOnInit$, animated$, visible$: requestedVisible$, onVisibleChange$, onHidden$, onShown$, horizontal$, ...stateProps }, patch] = stores.writablesForProps(defaultCollapseConfig, config, commonCollapseConfigValidator);
  ...
  return {
    ...
    directives: {
      transitionDirective: directive.bindDirectiveNoArg(transition.directives.directive)
    },
    actions: {}
  };
}
