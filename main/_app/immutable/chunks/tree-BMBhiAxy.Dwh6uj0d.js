...
function createTree(config) {
  const [{ nodes$, onExpandToggle$, navSelector$, ariaLabelToggleFn$, ...stateProps }, patch] = utils_directive.writablesForProps(
    defaultTreeConfig,
    config,
    configValidator
  );
  ...
  const widget = {
    ...utils_directive.stateStores({ normalizedNodes$, expandedMap$, ...stateProps }),
    patch,
    api: {},
    directives: {
