...
const dom$1 = require("@floating-ui/dom");
...
function createSelect(config) {
  ...
  const id$ = tansu.computed(() => _dirtyId$() ?? dom.generateId());
  ...
  const widget = {
    ...
    directives: {
      ...
      floatingDirective,
      referenceDirective,
      inputContainerDirective: utils_directive.bindDirective(navDirective, navManagerConfig$)
    },
    actions: {
      onInput({ target }) {
