...
const createTransition = (config) => {
  const [{ animated$, initDone$, visible$: requestedVisible$, transition$, onShown$, onHidden$, onVisibleChange$, animatedOnInit$ }, patch] = utils_directive.writablesForProps(defaultValues, config, configValidator);
  const { element$, directive: storeDirective } = utils_directive.createStoreDirective();
  const elementPresent$ = tansu.computed(() => !!element$());
  const visible$ = utils_directive.bindableDerived(onVisibleChange$, [requestedVisible$], ([visible]) => visible);
