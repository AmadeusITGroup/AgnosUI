...
function createAccordionItem(config) {
  const [
    ...
    patch
  ] = utils_directive.writablesForProps(defaultItemConfig, config, configItemValidator);
  const initDone$ = tansu.writable(false);
  const id$ = utils_directive.idWithDefault(_dirtyId$);
...
function factoryCreateAccordion(itemFactory = createAccordionItem, accordionItemProps = coreAccordionItemProps, accordionConfig = defaultAccordionConfig, accordionValidator = configAccordionValidator) {
  return (config) => {
    const [writables, patch] = utils_directive.writablesForProps(accordionConfig, config, accordionValidator);
    ...
    return {
      ...utils_directive.stateStores({ itemWidgets$, className$ }),
      patch,
      api: {
        expand: (id) => {
          var _a;
        ...
        registerItem: (propsConfig) => {
          ...
          const config2 = utils_directive.mergeConfigStores(itemProps, utils_directive.normalizeConfigStores(itemProps, propsConfig == null ? void 0 : propsConfig.config), accordionItemConfig);
          const [{ onHidden$, onShown$ }] = utils_directive.writablesForProps(
            {
