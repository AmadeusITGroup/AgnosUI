...
const extendWidgetProps = (factory, extraPropsDefaults, extraPropsConfig, overrideDefaults) => (propsConfig) => {
  const extraPropsWritables = utils_directive.writablesWithDefault(extraPropsDefaults, propsConfig, extraPropsConfig);
  const propsConfigConfig = propsConfig == null ? void 0 : propsConfig.config;
  const config = utils_directive.isStore(propsConfigConfig) ? tansu.computed(() => ({ ...overrideDefaults, ...propsConfigConfig() })) : { ...overrideDefaults, ...propsConfigConfig ?? {} };
  const widget = factory({ props: propsConfig == null ? void 0 : propsConfig.props, config });
  return {
    ...widget,
    ...utils_directive.stateStores({ ...widget.stores, ...extraPropsWritables }),
