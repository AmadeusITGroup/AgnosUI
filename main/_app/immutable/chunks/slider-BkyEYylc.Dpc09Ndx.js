...
function createSlider(config) {
  ...
  const { directive: sliderDirective, element$: sliderDom$ } = utils_directive.createStoreDirective();
  const { directive: minLabelDomDirective, element$: minLabelDom$ } = utils_directive.createStoreDirective();
  const { directive: maxLabelDomDirective, element$: maxLabelDom$ } = utils_directive.createStoreDirective();
  ...
  const minValueLabelDisplay$ = tansu.computed(() => {
    ...
    return rtl$() ? !valuesPercent$().some((percent) => 100 - percent > 100 - minLabelWidth - 1) : !valuesPercent$().some((percent) => percent < minLabelWidth + 1);
  });
  const maxValueLabelDisplay$ = tansu.computed(() => {
    ...
    return rtl$() ? !valuesPercent$().some((percent) => 100 - percent < maxLabelWidth + 1) : !valuesPercent$().some((percent) => percent > 100 - maxLabelWidth - 1);
  });
  ...
  const widget = {
    ...
    directives: {
      ...
      progressDisplayDirective: utils_directive.createAttributesDirective((progressContext$) => ({
        styles: {
          left: tansu.computed(() => progressContext$().option.left + "%"),
          right: tansu.computed(() => progressContext$().option.right + "%"),
          top: tansu.computed(() => progressContext$().option.top + "%"),
          bottom: tansu.computed(() => progressContext$().option.bottom + "%"),
          width: tansu.computed(() => progressContext$().option.width + "%"),
          height: tansu.computed(() => progressContext$().option.height + "%")
        },
        classNames: {
      ...
      handleDirective: utils_directive.createAttributesDirective((handleContext$) => ({
        ...
        styles: {
          left: tansu.computed(() => handleDisplayOptions$()[handleContext$().item.id].left + "%"),
          top: tansu.computed(() => handleDisplayOptions$()[handleContext$().item.id].top + "%")
        },
        classNames: {
      ...
      combinedHandleLabelDisplayDirective: utils_directive.createAttributesDirective(() => ({
        ...
        styles: {
          left: tansu.computed(() => combinedLabelPositionLeft$() + "%"),
          top: tansu.computed(() => combinedLabelPositionTop$() + "%")
        }
      })),
      handleLabelDisplayDirective: utils_directive.createAttributesDirective((labelDisplayContext$) => ({
        ...
        styles: {
          left: tansu.computed(() => handleDisplayOptions$()[labelDisplayContext$().index].left + "%"),
          top: tansu.computed(() => handleDisplayOptions$()[labelDisplayContext$().index].top + "%")
        }
      }))
    },
    actions: {
