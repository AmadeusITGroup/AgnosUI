...
const ModalBody = ({widget, state}: ModalContext<void>) => (
	<>
		...
		<button
			...
			onClick={() => void openModal({title: 'Other modal', children: ModalBody, container: state.modalElement})}
		>
			Launch other modal
		</button>
		<br />
...
const StackDemo = () => (
	<button className="btn btn-primary" onClick={() => void openModal({title: 'First modal', className: 'modal-sm', children: ModalBody})}>
		Launch demo modal
	</button>
);
