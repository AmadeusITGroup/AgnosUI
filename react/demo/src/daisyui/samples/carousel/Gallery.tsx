...
import {type CarouselApi, type CarouselDirectives, createCarousel} from '@agnos-ui/react-headless/components/carousel';
...
const ImageSlide = ({
	...
	id,
	registerSlide,
}: Photo & {loadRequested: boolean; aspectRatio: number; id: string; registerSlide: CarouselApi['registerSlide']}) => {
	const spinnerRef = useRef<HTMLSpanElement>(null);
	const [imgLoaded, setImgLoaded] = useState(false);
	const {
		directives: {root},
	} = useWidgetWithConfig(registerSlide, {id}, null);
	return (
		<div className="relative basis-full min-w-0 shrink-0 grow-0 flex justify-center" {...useDirective(root)}>
			{loadRequested ? (
				<>
					{!imgLoaded && (
...
const GalleryNavArrows = ({slidePrev, slideNext}: {slidePrev: CarouselDirectives['slidePrev']; slideNext: CarouselDirectives['slideNext']}) => (
	<div className="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
		<button
			className="btn btn-primary btn-sm md:btn-md btn-circle opacity-75 hover:opacity-100"
			onPointerDown={(e) => e.preventDefault()}
			{...useDirective(slidePrev)}
		>
			...
			<div className="overflow-hidden relative cursor-grab active:cursor-grabbing" {...useDirective(mainDirectives.root)}>
				<div className="flex max-h-full">
					{photosWithLoadState.map((photoWithLoadState, index) => (
						<ImageSlide key={index} {...photoWithLoadState} aspectRatio={aspectRatio} id={`gallery-photo-${index}`} registerSlide={registerSlide} />
					))}
				</div>
				{withNavArrows && <GalleryNavArrows slideNext={mainDirectives.slideNext} slidePrev={mainDirectives.slidePrev} />}
				{canFullScreen && (
