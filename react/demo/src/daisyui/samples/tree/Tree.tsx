...
export function Tree(props: Partial<Pick<TreeProps, 'className' | 'nodes'>>) {
	const {
		state,
		directives: {navigationDirective, itemToggleDirective},
	} = useWidgetWithConfig(createTree, props, 'tree');
	return (
		<ul role="tree" className={classNames('au-tree', state.className)} {...useDirective(navigationDirective)}>
...
const ToggleButtonDisplay = ({directive, item}: {directive: TreeDirectives['itemToggleDirective']; item: NormalizedTreeItem}) => {
	return (
		<button className="ms-auto" {...useDirective(directive, {item})}>
	...
	return (
		<li>
			<span className="flex flex-wrap items-center">
				...
				{item.children.length > 0 && <ToggleButtonDisplay item={item} directive={itemToggleDirective} />}
			</span>
