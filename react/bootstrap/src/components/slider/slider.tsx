...
const TickLabelDisplay = ({directive, tick}: {directive: SliderDirectives['tickLabelDirective']; tick: SliderTick}) => {
	return <span {...useDirective<{item: SliderTick}>(directive, {item: tick})}>{tick.value}</span>;
};
...
export const SliderDefaultTick = (slotContext: SliderSlotTickContext) => {
	return (
		<>
			...
			<span {...useDirective<{item: SliderTick}>(slotContext.directives.tickDirective, {item: slotContext.tick})}>
	...
	<>
		...
		{slotContext.state.ticks.map((tick) => (
			<React.Fragment key={tick.position}>
				<Slot slotContent={slotContext.state.tick} props={{tick, ...slotContext}} />
			</React.Fragment>
		))}
