...
@Component({
	...
	template: ` <ng-template auToastStructure #structure let-state="state" let-api="api" let-actions="actions" let-directives="directives">
		@if (state.header) {
			<div class="toast-header">
				<ng-template [auSlot]="state.header" [auSlotProps]="{state, api, actions, directives}"></ng-template>
...
@Component({
	...
	template: ` <ng-template [auContentAsSlot]="defaultSlots">
		...
		@if (!state().hidden) {
			...
			>
				<ng-template [auSlot]="state().structure" [auSlotProps]="{state: state(), api, actions, directives}"></ng-template>
			</div>
		}`,
})
