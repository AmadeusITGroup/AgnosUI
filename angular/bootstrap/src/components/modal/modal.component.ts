...
@Component({
	...
	template: `
		<ng-template auModalHeader #header let-state="state" let-api="api" let-actions="actions" let-directives="directives">
			<h5 class="modal-title">
				<ng-template [auSlot]="state.title" [auSlotProps]="{state, actions, api, directives}"></ng-template>
			</h5>
			@if (state.closeButton) {
				<button class="btn-close" [auUse]="directives.closeButtonDirective"></button>
			}
		</ng-template>
		<ng-template auModalStructure #structure let-state="state" let-api="api" let-actions="actions" let-directives="directives">
			@if (state.title) {
				<div class="modal-header">
					<ng-template [auSlot]="state.header" [auSlotProps]="{state, actions, api, directives}"></ng-template>
				</div>
			}
			...
			@if (state.footer) {
				<div class="modal-footer">
					<ng-template [auSlot]="state.footer" [auSlotProps]="{state, actions, api, directives}"></ng-template>
				</div>
			}
		</ng-template>
	`,
