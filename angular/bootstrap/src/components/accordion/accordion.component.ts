...
@Component({
	...
	template: `
		<ng-template #structure let-state="state" let-directives="directives" let-api="api" let-actions="actions">
			...
			<ng-template #button>
				<button class="accordion-button " [auUse]="directives.buttonDirective">
					<ng-template [auSlotProps]="{state, api, actions, directives}" [auSlot]="state.header"></ng-template>
				</button>
			</ng-template>
			@if (state.shouldBeInDOM) {
				<div [auUse]="directives.bodyContainerDirective" class="accordion-collapse">
					<div class="accordion-body" [auUse]="directives.bodyDirective">
						<ng-template [auSlotProps]="{state, api, actions, directives}" [auSlot]="state.children"></ng-template>
					</div>
				</div>
			}
		</ng-template>
...
@Component({
	...
	template: `
		...
		<ng-template [auSlotProps]="{state: state(), api, actions, directives}" [auSlot]="state().structure"></ng-template>
	`,
})
