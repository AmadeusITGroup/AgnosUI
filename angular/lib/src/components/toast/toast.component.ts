...
@Component({
	...
	template: ` <ng-template [auSlotDefault]="defaultSlots">
		...
		@if (!state().hidden) {
			<div [auUse]="toastDirective" class="au-toast toast {{ state().className }}" role="alert" aria-atomic="true">
...
export class ToastComponent extends BaseWidgetDirective<ToastWidget> implements AfterContentChecked {
	...
	@CachedProperty
	get toastDirective() {
		return mergeDirectives(this._widget.directives.toastDirective, this._widget.directives.transitionDirective);
	}
