...
import {TmplAstElement, TmplAstTemplate, parseTemplate as parseTemplateUsingCompiler} from '@angular/compiler';
 ...
 * @returns The updated template html.
 */
export function updateAttribute(html: string, node: TmplAstElement, name: string, update: (old: string | null) => string | null): string {
 ...
 * @param removing the number of characters to remove
 * @param newSubstr A new substring.
 * @returns the updated string.
 */
export function replaceAt(str: string, offset: number, removing: number, newSubstr: string): string {
	const index = offset;
	const prefix = str.slice(0, index);
	const suffix = str.slice(index + removing);
