 ...
 * @param min - Minimum boundary value that any value in the array cannot go below
 * @param max - Maximum boundary value that any value in the array cannot exceed
...
export const manageMinMaxRange = (
	...
	if (minRange) {
		pushHandle(values, index, currentValue, 1, minRange, min, max, pushRange);
		pushHandle(values, index, currentValue, -1, minRange, min, max, pushRange);
	}
	if (maxRange) {
		pullHandle(values, index, currentValue, 1, maxRange, min, max, pushRange);
		pullHandle(values, index, currentValue, -1, maxRange, min, max, pushRange);
	}
	return values;
};
