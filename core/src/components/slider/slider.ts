...
export const createSlider: WidgetFactory<SliderWidget> = createWidgetFactory('slider', (config?: PropsConfig<SliderProps>) => {
	...
	const minLabelSize$ = computed(() => pixelToPercent(minLabelDomRect$()[vertical$() ? 'height' : 'width']));
	const maxLabelSize$ = computed(() => pixelToPercent(maxLabelDomRect$()[vertical$() ? 'height' : 'width']));
	...
	const combinedLabelSize$ = computed(() => pixelToPercent(combinedDimensions$()?.contentRect[vertical$() ? 'height' : 'width']));
	...
	const currentLabelSizeByIndex = (index: number) =>
		pixelToPercent(currentLabelDoms$()[index]?.getBoundingClientRect()[vertical$() ? 'height' : 'width']);
	...
	function doLabelsIntersect(): boolean {
		...
		const labelPosition1 = labelPosition(handleOptions[0][dimension]!, currentLabelSizeByIndex(0));
		const labelPosition2 = labelPosition(handleOptions[1][dimension]!, currentLabelSizeByIndex(1));
