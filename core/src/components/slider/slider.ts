...
export function createSlider(config?: PropsConfig<SliderProps>): SliderWidget {
	...
	const minLabelSize$ = computed(() => {
		...
		return ((vertical$() ? minLabelDomRect.height : minLabelDomRect.width) / sliderDomRectSize$()) * 100;
	});
	const maxLabelSize$ = computed(() => {
		const maxLabelDomRect = maxLabelDomRect$();
		return ((vertical$() ? maxLabelDomRect.height : maxLabelDomRect.width) / sliderDomRectSize$()) * 100;
	});
	...
	const combinedLabelDisplay$ = computed(() => {
		...
		return values.length == 2 && values[1] - secondLabelSize - values[0] + firstLabelSize < (Math.max(firstLabelSize, secondLabelSize) * 2 || 10);
	});
