import {idWithDefault, stateStores, true$, writablesForProps} from '../../utils/stores';
import type {ConfigValidator, Directive, PropsConfig, Widget} from '../../types';
import {bindDirective, browserDirective, createAttributesDirective, mergeDirectives, registrationArray} from '../../utils/directive';
...
export interface CarouselState extends CarouselCommonPropsState {
	...
	/**
	 * all slide ids inside the carousel container
	 */
	slides: ({id: string} & CarouselSlideWidget)[];
	/**
...
/**
 * Represents the API for an Embla
 */
interface EmblaApi {
	/**
 ...
 */
export interface CarouselApi extends EmblaApi {
	/**
	 * Creates a new carousel slide
	 * @param config - optional slide config
	 * @returns the registered carousel slide widget
	 */
	registerSlide: (config?: PropsConfig<CarouselSlideProps>) => CarouselSlideWidget;
}
...
interface CarouselSlideCommonPropsAndState {
	/**
	 * the id of the slide
	 * @defaultValue `''`
	 */
...
/**
 * Represents the properties for a carousel slide.
 */
export interface CarouselSlideProps extends CarouselSlideCommonPropsAndState {}
 ...
 */
export interface CarouselSlideDirectives {
	/**
	 * the slide directive
	 */
	root: Directive;
...
/**
 * Represents a carousel slide widget with specific properties, state, API, and directives.
 */
export type CarouselSlideWidget = Widget<CarouselSlideProps, CarouselSlideState, object, CarouselSlideDirectives>;

const defaultSlideConfig: CarouselSlideProps = {
	id: '',
};

/**
 * Retrieve a shallow copy of the default CarouselSlide config
 * @returns the default CarouselSlide config
 */
export function getCarouselSlideDefaultConfig(): CarouselSlideProps {
	return {...defaultSlideConfig};
}

const configSlideValidator: ConfigValidator<CarouselSlideProps> = {
	id: typeString,
};
...
/**
 * Create an CarouselSlideWidget with given config props
 * @param selected$ - if the carousel slide is selected
 * @param showNavigationIndicators$ - whether the navigation indicators are visible
 * @param config - an optional carousel slide config
 * @returns a CarouselSlideWidget
 */
export function createCarouselSlide(
	selected$: ReadableSignal<boolean>,
	showNavigationIndicators$: ReadableSignal<boolean>,
	config?: PropsConfig<CarouselSlideProps>,
): CarouselSlideWidget {
	const [{id$: _dirtyId$}, patch] = writablesForProps(defaultSlideConfig, config, configSlideValidator);
	...
	return {
		...
		api: {},
		directives: {
			root: createAttributesDirective(() => ({
				attributes: {
					id: id$,
...
export function createCarousel(config?: PropsConfig<CarouselProps>): CarouselWidget {
	...
	const slideWidgets$ = registrationArray<CarouselSlideWidget>();

	return {
		...stateStores({
			...emblaCarousel.stores,
			...stateProps,
			slides$: computed(() => slideWidgets$().map((slide) => ({id: slide.stores.id$(), ...slide}))),
			direction$,
			showNavigationIndicators$,
		}),
		patch,
		api: {
			...emblaCarousel.api,
			registerSlide: (config?: PropsConfig<CarouselSlideProps>) => {
				const slide = createCarouselSlide(
					computed(() => slideWidgets$()[emblaCarousel.stores.selectedScrollSnap$()] === slide),
					showNavigationIndicators$,
					config,
				);
				slide.directives.root = mergeDirectives(
					() => ({
						destroy: slideWidgets$.register(slide),
					}),
					slide.directives.root,
				);
				return slide;
			},
		},
