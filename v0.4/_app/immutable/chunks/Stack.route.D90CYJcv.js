const o=`import type {ModalContext} from '@agnos-ui/react-bootstrap/components/modal';
...
const ModalBody = ({widget, state}: ModalContext<void>) => (
	<>
		...
		<button
			...
			onClick={() => openModal({slotTitle: 'Other modal', children: ModalBody, container: state.modalElement})}
		>
			Launch other modal
		</button>
		<br />
...
const StackDemo = () => (
	<button className="btn btn-primary" onClick={() => openModal({slotTitle: 'First modal', className: 'modal-sm', children: ModalBody})}>
		Launch demo modal
	</button>
);
