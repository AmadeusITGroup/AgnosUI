	...
	<div class="overflow-hidden relative cursor-grab active:cursor-grabbing" use:mainDirectives.root>
		<div class="flex max-h-full">
			{#each photosWithLoadState as { src, alt, sources, loadRequested }, index (index)}
				<GalleryImage {src} {alt} {sources} {loadRequested} {aspectRatio} id="gallery-photo-{index}" {registerSlide} />
			{/each}
		</div>
		{#if withNavArrows}
			<div class="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
				<button class="btn btn-primary btn-sm md:btn-md btn-circle opacity-75 hover:opacity-100" {onpointerdown} use:mainDirectives.slidePrev>
					{@html previousSvg}
				</button>
				<button class="btn btn-primary btn-sm md:btn-md btn-circle opacity-75 hover:opacity-100" {onpointerdown} use:mainDirectives.slideNext>
					{@html nextSvg}
				</button>
			</div>
		{/if}
