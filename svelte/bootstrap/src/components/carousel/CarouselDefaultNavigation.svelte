<script lang="ts" generics="SlideData extends {id: string;}">
	...
	let {state, attachments}: CarouselContext<SlideData> = $props();
</script>
...
{#if state.showNavigationArrows}
	{#if state.canScrollPrev}
		...
		<button class={prevBtnClassPrefix} {@attach attachments.scrollPrev()}>
	...
	{#if state.canScrollNext}
		...
		<!-- svelte-ignore a11y_consider_explicit_label -->
		<button class={nextBtnClassPrefix} {@attach attachments.scrollNext()}>
			<span class="{nextBtnClassPrefix}-icon"></span>
		</button>
	{/if}
{/if}
{#if state.showNavigationIndicators}
	<div class="carousel-indicators" {@attach attachments.tabList()}>
		{#each state.slidesData as { id }, index (id)}
			<!-- svelte-ignore a11y_consider_explicit_label -->
			<button data-bs-target="true" class={[state.selectedScrollSnap === index && 'active']} {@attach attachments.tabIndicator({index, id})}></button>
		{/each}
	</div>
{/if}
