...
test.describe(`Drawer tests`, () => {
	...
	test(`Drawer should be resizable and save the new size`, async ({page}) => {
		...
		expect(await drawerPO.statePosition()).toStrictEqual(assign(expectedVariables, {'--df-drawer-size': `${expectedBoundingBox.width}px`}));
		...
		await expect.poll(async () => await drawerPO.locatorRoot.boundingBox()).toStrictEqual(assign(expectedBoundingBox, {width: 1280, height: 150}));
		expect(await drawerPO.statePosition()).toStrictEqual(assign(expectedVariables, {'--df-drawer-size': '150px'}));
		...
		expect(await drawerPO.statePosition()).toStrictEqual(assign(expectedVariables, {'--df-drawer-size': '304px'}));
